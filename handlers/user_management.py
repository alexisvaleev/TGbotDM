...
async def cmd_view_users(message: types.Message):
    ...
    text = "\n".join(
        f"{u.tg_id}: {u.surname or '-'} {u.name or '-'} {u.patronymic or '-'} ({u.role})"
        for u in users
...
async def start_add_user(message: types.Message, state: FSMContext):
    ...
    await message.answer("Введите Telegram ID нового пользователя:", reply_markup=ReplyKeyboardRemove())
    ...
    async with AsyncSessionLocal() as s:
        ...
        if ex:
            await s.execute(update(User)
                            .where(User.tg_id==new_id)
                            .values(role=txt))
            msg = f"✅ Пользователь {new_id} обновлён в «{txt}»."
        else:
            await s.execute(insert(User).values(
                tg_id=new_id, role=txt,
                surname=None, name=None, patronymic=None
            ))
            msg = f"✅ Пользователь {new_id} создан как «{txt}»."
        await s.commit()
    await state.finish()
...
async def add_users_to_db():
    """
    Сидирование ADMIN_IDS, TEACHER_IDS, STUDENT_IDS → users
    """
    cfg = load_config()
    async with AsyncSessionLocal() as s:
...
def register_user_management(dp: Dispatcher):
    dp.register_message_handler(cmd_view_users,
                                text="Просмотр пользователей", state="*")
